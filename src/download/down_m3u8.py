...
def main(save_audio, save_mp4, save_ts):
    ...
    global download
    download = M3u8Download(path, name, url_=url_, headers=headers, progressbar=progressbar, print_log=print_log)
...
class M3u8Download():
    ...
    def __init__(self, path: str, name: str, url_: str = "", headers="", progressbar=progressbar, print_log=print_log) -> None:
        self.solve_count = 0
        self.path = path
    ...
    async def async_download_ts(self, url: str, i, session):  # 根据url下载ts文件
        ...
        try:
            if not os.path.exists(filename) or (os.path.getsize(filename) == 0):  # 如果存在或者大小为0就下载
            ...
            async with aiofiles.open(self.filem3u8, 'r', encoding='utf-8') as f:
                async for line in f:
                    ...
                    if not line.startswith("http"):
                        if self.url_ != '':
                            url = f"{self.url_}/{line}"
                        else:
                            temp = url_m3u8.split('/')
                            url = f"{temp[0]}//{temp[2]}/{line}"
                    else:
                        url = line
...
# 视频处理模块
class ProcessVideo():
    def __init__(self, path, name, progressbar=progressbar, print_log=print_log) -> None:
        self.name = name
        self.path = path
        self.path_m3u8 = path + '/m3u8'
        self.progressbar = progressbar
    ...
    def merge_paln_b(self, if_png):  # 暂时存在错误
        if (input('\n是否需要转成mp4(y/n)') == 'y'):  # ts直接合并成mp4
            self.merge_ts_video_useffmpeg(f'{self.path}/{self.name}', f'{self.path}/{self.name}.mp4', if_png)
            if (input('\n是否需要生成mp3(y/n)') == 'y'):
                self.mp4_to_m4a(self.name)
        else:  # ts合并
            self.merge_ts(f'{self.path}/{self.name}', f'{self.path}/{self.name}.ts', if_png)
        if (input('\n是否删除ts文件(y/n)') == 'y'):
    ...
    def merge_paln_a(self, descrypt_path=None):  # 先合成一个ts，再转mp4
        ...
        if (self.check_withpngend()):
            ...
            self.merge_ts_endwithpng(f'{self.path}/{name_t}', f'{self.path}/{self.name}.ts')
        else:
            self.print_log("开始合并")
...
class Decrypt():
            ...
            for line in f:
                ...
                if (i == 6):
                    ...
                    if (line_s[0] == "#EXT-X-KEY:METHOD"):
                        ...
                        if not line.startswith("http"):
                            temp = url_m3u8.rsplit('/', 1)[0]
                            key_url = f"{temp}/{key_url}"
    ...
    async def dec_ts(self, file):
        ...
        if not my_file.is_file():
            async with aiofiles.open(f"{path}/{name}/{file}", mode="rb") as f1, aiofiles.open(f"{path}/{name}decrypted/{file}", mode="wb") as f2:
...
def url_conversion(url: str):
    try:
        if url.endswith('.com') or url.endswith('.com/'):
            ...
            response = requests.get(url)
            id = re.findall(r'replace\("(.*?)"\)', response.text, re.S)[0].rsplit('/', 1)[-1]
            response = requests.get(f"https://a.5865865.xyz/api.php?act=geturl9&id={id}")
